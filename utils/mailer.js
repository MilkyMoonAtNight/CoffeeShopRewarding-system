...
<body style="margin:0;padding:0;background:#f5ede4;font-family:'Helvetica Neue',Arial,sans-serif;">
    ...
    <tr><td align="center">
      <table width="560" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        <tr>
          <td style="background:#581217;padding:24px 32px;text-align:center;">
            <p style="margin:0;font-size:22px;font-weight:700;color:#fdf6ed;letter-spacing:0.03em;">☕ Con Leche</p>
            <p style="margin:6px 0 0;font-size:13px;color:rgba(253,246,237,0.65);">New message from the website</p>
          </td>
        </tr>
        <tr>
          <td style="padding:28px 32px 8px;">
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#fdf6ed;border-radius:10px;">
              <tr><td style="padding:16px 18px;">
                ...
                <p style="margin:4px 0 0;font-size:15px;font-weight:600;color:#2c2c2c;">${safe(name)}</p>
                <p style="margin:2px 0 0;font-size:13px;color:#888;">${safe(email)}${phone ? ' · ' + safe(phone) : ''}</p>
              </td></tr>
            </table>
          </td>
        </tr>
        <tr>
          <td style="padding:16px 32px 8px;">
            <p style="margin:0 0 8px;font-size:11px;text-transform:uppercase;letter-spacing:0.12em;color:#aaa;">Message</p>
            <p style="margin:0;font-size:15px;color:#2c2c2c;line-height:1.7;white-space:pre-line;">${safe(message)}</p>
          </td>
        </tr>
        <tr>
          <td style="padding:24px 32px;text-align:center;border-top:1px solid #f0e8df;margin-top:20px;">
            <p style="margin:0;font-size:11px;color:#bbb;">Reply directly to this email to respond to ${safe(name)}.</p>
          </td>
        </tr>
      </table>
    </td></tr>
  ...
  await getTransporter().sendMail({
    ...
    replyTo: email,
    subject: `✉️ Website message from ${name}`,
    html,
  });
}
